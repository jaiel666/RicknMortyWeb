...
class Api
{
    ...
    public function fetchCharactersForEpisode(int $id): array
    {
        $response = $this->client->get(self::API_URL . "/{$id}");
        $episodeData = json_decode((string)$response->getBody());

        $characterURLs = $episodeData->characters;
    ...
    public function fetchEpisodeByCode(string $code): ?Episode
    {
        $episodes = $this->fetchEpisodes();

        foreach ($episodes as $episode) {
            if ($episode->getEpisode() === $code) {
                return $episode;
            }
        }
